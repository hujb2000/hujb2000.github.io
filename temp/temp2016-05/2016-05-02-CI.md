...
cd /root/
source openrc
qa=`keystone user-role-list --user hzqiushou4 --tenant Project_hzqiushou4 | grep qa | wc -l`
if [ $qa -eq 1 ]; then
    keystone user-role-remove --user hzqiushou4 --role qa --tenant Project_hzqiushou4
fi
cd /root/tempest_netease/tempest-ci/tempest
nosetests -sv tempest/api/network
...
1. 密码加固
2. 访问IP白名单
3. 安全矩阵表(Matrix Authorization Strategy Plugin)
4. Jenkins.io
